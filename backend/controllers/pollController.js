...
exports.createPoll = async (req,res) => {
    ...
    if(!question || !type || !creatorId){
        return res
         .status(400)
         .json({ message: "Question, type, and creatorId are qrequired."});
    }
    try{
           ...
           switch (type){
            ...
            case "open-ended":
            processedOptions = [];   //No options neede for open ended.
            break;
        ...
        res
         .status(500)
         .jaon({ message: "Error registring user", error: err.message });
    }
}
...
exports.getVotedPolls = async (req,res) => {
  ...
  const userId = req.user._id;
    ...
    try{
          ...
          const polls = await Poll.find({ voters: userId }) //Filterby polls where the user's ID exists in the voters
...
exports.voteOnPoll = async (req,res) => {
    ...
    try{
        ...
        if (poll.closed){
            return res.status(400).json({ message: "Pollis closed."});
        }
